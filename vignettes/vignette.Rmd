...
```{r}
SCNLP <- StanfordCoreNLP$new(properties = props, output_format = "conll")
```
...
```{r}
conll_files <- lapply(segdirs, SCNLP$process_files)
```
...
df <- rbindlist(lapply(unlist(conll_files), fread, blank.lines.skip = TRUE, quote = ""))
```
...
dt <- segment(x = reuters_dt, dir = tempdir(), chunksize = 10L) %>%
  lapply(SCNLP$process_files) %>%
  unlist() %>%
  lapply(fread, blank.lines.skip = TRUE, quote = "") %>%
  rbindlist()
```
...
reuters_json <- J$annotate(reuters_dt[1][["text"]])
...
reuters_xml <- X$annotate(reuters_dt[1][["text"]])
y <- coreNLP::getToken(coreNLP:::parseAnnoXML(reuters_xml))
```
...
reuters_conll <- C$annotate(reuters_dt[1][["text"]])
```
