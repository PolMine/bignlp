...
```{r}
filenames_ndjson <- corenlp_annotate(input = tsv_file, output = ndjson_file)
```

The NDJSON output is parsed into a table ... 

```{r}
filenames_tsv_tagged <- corenlp_parse_ndjson(input = ndjson_file, output = tsv_file_tagged)
```
...
dt <- corenlp_annotate(input = tsv_file, output = ndjson_file) %>% 
  corenlp_parse_ndjson(output = tsv_file_tagged) %>%
  lapply(fread) %>%
  rbindlist()
```
...
A potentially big, confusing issue is that Stanford CoreNLP requires Oracle Java v1.8+, see [this respective note](https://stanfordnlp.github.io/CoreNLP/). The statement that Oracle Java is needed is not too obvious. But trying to use OpenJDK in combination with CoreNLP does not work, but you do not see immediately what might be the problem.

Oracle Java can be installed from [this website](https://www.oracle.com/technetwork/java/javase/downloads/jdk8-downloads-2133151.html). Note that Oracle licenses have become more restrictive, but this will not be a problem for academic/non-commercial contexts.

Once you have Oracle Java 1.8 on your system, making this Java version the one that is actually used may be a bit tricky. For macOS, we found the jEnv tool to be useful, see [this link](https://medium.com/@danielnenkov/multiple-jdk-versions-on-mac-os-x-with-jenv-5ea5522ddc9b).

To make R use Java 1.8, you may have to call R CMD javareconf from the command line.

```{sh}
R CMD javareconf
```

To check within R that everything works, run this snippet.
